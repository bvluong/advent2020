...
const checkForShinyGold = (map, key, shinyBagVal) => {
    let hasShinyGold = false;
    if (map[key]) {
        Object.keys(map[key]).forEach(val => {
            if (val === 'shiny gold' || shinyBagVal[val]) {
                shinyBagVal[key] = true;
                hasShinyGold = true;
            } else if (map[val]) {
                hasShinyGold = checkForShinyGold(map, val, shinyBagVal)
            }
        })
    }
    return hasShinyGold;
...
const getNumBags = (map, key, count) => {
    if (parseInt(map[key])) {
    ...
    let total = 0;
    Object.keys(map[key]).forEach(val => {
        const num = parseInt(map[key][val])
        if (num) {
            const bags = getNumBags(map, val);
            map[val] = bags
            total += (num * bags) + num
        }
    })
...
const stripText = (text) =>
    text.replace('.', '').replaceAll(' bags', '').replaceAll(' bag', '')

const findSolution = () => {
    let map = {}
    input.forEach(row => {
        const arr = row.split(' bags contain ')
        if (!map[arr[0]]) {
            map[arr[0]] = {}

        }
        if (arr[1] && arr[1].includes('other')) {
            map[arr[0]] = 0;
        } else if (arr[1]) {
            stripText(arr[1]).split(', ').forEach(y => {
                let count = y[0];
                let color = y.slice(2);
                map[arr[0]][color] ? map[arr[0]][color] += count : map[arr[0]][color] = count
            })
    ...
    console.log(map['shiny gold']);
}

const input = `dotted blue bags contain 5 wavy green bags, 3 pale beige bags.
